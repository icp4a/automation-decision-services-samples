...
public class Person {
    ...
    public String getInitials() {
        if (getCountry().equals(Country.JAPAN))
            return getFullName();
        String initials = firstLetter(getFirstName());
        if (getLastName().isEmpty())
            return initials;
        return firstLetter(getFirstName()) + firstLetter(getLastName());
    }
    ...
    private void parseName() {
        ...
        for (int i = 0; i < names.length; i++) {
            String capitalName = capitalizeFirstLetter(names[i]);
            if (i == 0)
