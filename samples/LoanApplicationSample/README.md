...
## Description
This sample shows a web application calling a decision service built into Decision Intelligence Client Managed Software. It provides code for a web-based client application 
which calls a decision service archive to process loan requests using Decision Intelligence runtime.

## Learning objectives
- Deploy a decision service archive to a production environment.
- Configure a web application to call a decision service archive using the Decision Intelligence runtime.
- Run the web application with representative data.
- Explore the execution trace.

## Audience

This sample is for anyone who wants to execute a decision service archive built with Decision Intelligence.
...
## Prerequisites
- Decision Intelligence: Your instance of the services must have a runtime that supports basic authentication. You must have the host name and login credentials (user name and password) allowing management and execution in the runtime. 
...
- Websphere Application Server Liberty: A Java application server that you can download from [Download WAS Liberty](https://www.ibm.com/support/pages/websphere-liberty-developers/). This sample was tested on **WebSphere Liberty Web Profile 8 26.0.0.1**.
...
You use the Swagger UI tool to explore the contents of a deployment space and deploy if needed the decision service archive. The name chosen for the deployment space in this tutorial is `DIsample`.
...
You use the Swagger UI tool of the runtime to deploy the decision service archive in the runtime `DIsample` storage.
...
8. Click on Execute.<br>
   Note: Execution may fail as the cache used by the Decision Intelligence runtime is out of date. Retry an execution afterward (see the parameter `decision_selection.update_interval` at `Decision runtime parameters`[![CP4BA](/resources/cloudpak4ba.svg "IBM Cloud Pak for Business Automation")](https://www.ibm.com/docs/en/cloud-paks/cp-biz-automation/26.0.0?topic=parameters-decision-runtime)[![ADS](/resources/ads.svg "IBM Decision Intelligence Client Managed Software")](https://www.ibm.com/docs/en/dicms/26.0.0?topic=parameters-decision-runtime) )
...
In this section, you download the repository for the sample application, set properties to match your Decision Intelligence runtime, and build the application WAR file.
...
2. Set the values notes as `TO BE SET` in the file `samples/LoanApplicationSample/src/main/webapp/resources/config.js`:
   - **SERVERNAME**: The name of the host server on which you run the Decision Intelligence runtime, for example, `runtime.di.com`.
   - **DIUSERNAME** and **DIPASSWORD**: The credentials allowing execution for your Decision Intelligence runtime.
   - **SPACEID**: `DIsample`.
...
- To remove the Liberty server, delete the directory ```<path to Liberty>/usr/servers/testDI```.
...
- To undeploy the archive:
    ...
    4. Give `DIsample` as the deploymentSpaceId and `loanApproval` as the decisionId .
    5. Click on Execute. The response code is 200, the archive is removed from the deployment space `DIsample`.
